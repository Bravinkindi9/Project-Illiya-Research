...
We propose an alternative. Rather than classifying pixels, we classify
buildings. Google Open Buildings V3 provides vector footprint polygons
for approximately 1.8 billion structures across Africa, Latin
America, and South Asia, extracted via a combination of aerial and satellite
imagery at confidence thresholds that permit large-scale reliable
retrieval. From these polygons, we engineer 15 spatial features that
encode the geometric and contextual properties of each building: its
size, shape regularity, orientation, and its spatial relationship to
...
## Preprint Closeout Status

The preprint is now structurally complete. Here is the section status after this session:
...
| 3 — Study Area & Data | *(pre-existing, not in scope)* |
| 4.1 — Feature Engineering | ✅ Approved |
| 4.4 — Validation Protocol | ✅ Approved |
| Table 1 | ✅ Drafted |
| 5 — Results | ✅ Drafted |
| 6 — Discussion | ✅ Drafted |
| 7 — Conclusion | ✅ Drafted |

Three items remain for your action: inject the cluster $N$ counts into Table 2, confirm the citation list against your reference manager (Wurm, Persello, Kropf, Fleischmann, Sirko, Arbelaitz, Huang are cited by name — verify years and DOIs), and decide whether to name *momepy* explicitly in Section 2.2 or remove it if it's not in your reference list. Everything else is ready for LaTeX typesetting.
